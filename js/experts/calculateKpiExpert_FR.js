...
calculateKpiExpert_FR.getTooltipDetail=function(entityId,varName){    
    ...
    for(var i=0;  i < fillRateEntities.length; i++){
        ...
        if(fillRateEntities[i].key.toLowerCase()==entityId.toLowerCase()){
            ...
            var text=`<div class="tooltipDetailElement"><img id="" src="images/mode_1.png" style="width:30px;padding-top:0px"></img>
            ...
            <span style='color:#ffffff;font-size:${12*escalaTextos}px;'>Vol. Entregado:</span> <span style='color:#ffffff;font-size:${12*escalaTextos}px;'>${Math.round((fillRateEntities[i][varName].totalVolumenEntregado/fillRateEntities[i][varName].totalSolicitado)*100)}% </span><span style='color:#ffffff;font-size:${12*escalaTextos}px;'>${formatNumber(fillRateEntities[i][varName].totalVolumenEntregado)} TM</span><br>
            <span style='color:#ffffff;font-size:${12*escalaTextos}px;'>Vol. Solicitado:</span> <span style='color:#ffffff;font-size:${12*escalaTextos}px;'>${formatNumber(fillRateEntities[i][varName].totalSolicitado)} TM</span><br>
            </div>
            `
            return text;
        }
