    ...
    for(var i=0;  i < entities.length; i++){
        ...
        if(entities[i].key.toLowerCase()==entityId.toLowerCase()){
            ...
            if(entities[i].produccion){
                ...
                <span style='color:#fff600;font-size:${15*escalaTextos}px;'> <span style='color:#ffffff'>${prodPer}</span> <span style='color:#ffffff;font-size:${12*escalaTextos}px;'> (Plan: ${formatNumber(entities[i].produccion.VolVenta_Plan)} TM , Real:${formatNumber(entities[i].produccion.VolVenta_Real)} TM)</span><br>
                </div>
                `
